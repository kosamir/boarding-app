...
//		createTemplate(2);
	...
	private void createTestTemplate() {
		...
		template = boardingRepsitory.findById(template.getId()).orElse(null);

		Form form3 = new Form();
		form3.setName("Hobiji");
		template.addForm(form3);

		Field field10 = new Field();
		field10.setLabel("Najdrazi hobi");
		field10.setType(FieldType.CHOICE);
		field10.setValue("Plivanje, Skakanje, Trcanje");
		form3.addField(field10);

		Field field11 = new Field();
		field11.setLabel("Najdraza zivotinja");
		field11.setType(FieldType.CHOICE);
		field11.setValue("Pcela, Konj, papiga");
		form3.addField(field11);

		form3.setNumOfFields(form3.getFields().size());
		template.setNumOfForms(template.getForms().size());
